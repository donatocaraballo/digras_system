    ...
    useEffect(() => {
        function handleClickOutside(event) {
            ...
            if (mobileMenuRef.current && !mobileMenuRef.current.contains(event.target) && showMobileMenu) {
                 // Opcional: Cerrar si se hace click fuera del drawer (aunque el overlay ya lo maneja)
            }
        }
        document.addEventListener("mousedown", handleClickOutside);
        return () => document.removeEventListener("mousedown", handleClickOutside);
    }, [profileRef, showMobileMenu]);
    ...
    return (
        <div style={styles.navWrapper}>
            <nav style={styles.island}>
                ...
                {/* 1. IZQUIERDA: LOGO + HAMBURGUESA (Móvil) */}
                    ...
                    <div style={styles.userInfo} onClick={() => setShowProfileMenu(!showProfileMenu)}>
                        ...
                        <div className="user-meta-visible" style={styles.userMetaVisible}>
                            ...
                            <div style={styles.subMetaRow}>
                                <span style={{...styles.visibleRole, color: roleStyle.color}}>{userRole}</span>
                            </div>
                        </div>
                                ...
                                return (
                                    <Link key={mod.name} to={mod.path} style={{ textDecoration: 'none' }} onClick={() => setShowMobileMenu(false)}>
                                        <div style={{
                                            ...
                                            color: isActive ? mod.color : '#64748b',
            ...
            <style>{`
                ...
                @media (max-width: 1024px) {
                    .user-meta-visible { display: none !important; } /* Ocultar nombre en tablet */
                }

                @media (max-width: 768px) {
                    .desktop-menu { display: none !important; } /* Ocultar menú central en móvil */
                    .mobile-only { display: flex !important; } /* Mostrar hamburguesa */
