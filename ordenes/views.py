...
from base.models import Orden, DetalleOrden, Existencia, Lote, Usuario
...
class OrdenViewSet(BaseViewSet):
    ...
    def _consumir_lotes_fifo(self, producto_id: int, cantidad_total: int):
        """
        Consume unidades usando lotes FIFO.
        - Toma los lotes más antiguos primero.
        - Descuenta cantidad hasta completar el total solicitado.
        - Si los lotes no alcanzan, lanza ValueError para activar rollback.
        - Este método asume que la existencia global ya fue descontada.
        """
        if cantidad_total <= 0:
            return
        ...
        lotes = (
            ...
            .filter(id_producto_id=producto_id, cantidad__gt=0)
            .order_by('fecha_pedido', 'id_lote')  # más antiguos primero
        )
        ...
        for lote in lotes:
            ...
            if disponible_lote >= restante:
                ...
                Lote.objects.filter(pk=lote.pk).update(
                    cantidad=F('cantidad') - restante
                )
                restante = 0
            else:
                # Consumimos todo el lote y seguimos con el siguiente
                Lote.objects.filter(pk=lote.pk).update(cantidad=0)
                restante -= disponible_lote

        if restante > 0:
            # Aquí hay una inconsistencia entre Existencia y Lotes
            raise ValueError(
                f"Stock de lotes insuficiente para el producto {producto_id}. "
                f"Faltan {restante} unidades en lotes."
            )

    def _devolver_a_lotes_fifo(self, producto_id: int, cantidad_total: int):
        """
        Devuelve unidades al sistema de lotes:
        - Selecciona el lote más reciente para devolver la mercancía.
        - Si no existen lotes, solo se actualiza Existencia desde afuera.
        """
        if cantidad_total <= 0:
            return
        ...
        lotes = (
            ...
            .order_by('-fecha_pedido', '-id_lote')  # más recientes primero
        )
        ...
        # Si no hay lotes, no hacemos nada. La Existencia ya se ajustó afuera.

    # ==========================================================
    #   CREAR ORDEN
    # ==========================================================
    # Crea una nueva orden verificando stock, descontando Existencia,
    # consumiendo lotes en FIFO, generando detalles y registrando acción.
    @transaction.atomic
    def create(self, request, *args, **kwargs):

        # Solo vendedores pueden crear órdenes
        if request.user.tipo != "VENDEDOR":
            return Response(
                {"error": "Solo los vendedores pueden crear órdenes."},
        ...
        # 1. Reunir cantidades requeridas por producto
        required = {}
        ...
        # 2. Bloquear existencias y validar stock
        existencias = (
            Existencia.objects.select_for_update()
            .filter(id_producto_id__in=required.keys())
        )
                ...
                return Response(
                    {
                        "detail": (
                            ...
                            f"Disponible: {ex.cantidad}, requerido: {qty_needed}"
                        )
                    },
                    status=status.HTTP_400_BAD_REQUEST
                )

        # 3. Descontar inventario (Existencia)
        for pid, qty_needed in required.items():
            ex = exist_map[pid]
            Existencia.objects.filter(pk=ex.pk).update(
                cantidad=F("cantidad") - qty_needed
            )
        ...
        try:
            ...
            # Cualquier error aquí hará rollback de toda la transacción
            return Response(
                {"detail": str(e)},
        ...
        orden = Orden.objects.create(
            ...
            id_cliente_id=datos["id_cliente"],   # si id_cliente viene como entero
        ...
        # 5. Crear detalles y calcular totales
        for item in detalles_data:
        ...
        registrar_accion(
            ...
            "Órdenes",
            "Crear orden",
            f"Orden creada con {len(detalles_data)} productos. Total: {precio_final}",
